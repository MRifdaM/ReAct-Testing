...
describe('Fitur Halaman Daftar Laporan Kerusakan', () => {
  ...
  const SELECTOR_DETAIL_BTN = 'button.btn-info'; // Selector tombol detail
  const MODAL_TITLE_TEXT_DETAIL = 'Detail Laporan';
  const BTN_TEXT_TUTUP = 'Tutup';
  
  // --- Konstanta Modal Detail (Input Fields) ---
  // Perhatikan: Ini adalah input 'disabled' di modal detail
  ...
  const SELECTOR_DETAIL_DESKRIPSI = '#myModal input#laporan_deskripsi'; // Asumsi ini adalah status

  // --- Konstanta Test Data ---
  const JUDUL_LAPORAN_TEST = 'Cypress Detail Test - Kursi Goyang RT02';
  const DATA_LANTAI_ID = '5';
  const DATA_RUANG_ID = '3'; // Ruang RT02
  const DATA_SARANA_ID = '67'; // Sarana Kursi di RT02
  const DATA_GEDUNG_VALUE = 'Teknik Sipil';
  const DATA_LANTAI_VALUE = 'Lantai 5';
  const DATA_RUANG_VALUE = 'Ruang Teori 02';
  const DATA_SARANA_VALUE_PARTIAL = 'SAR-'; // Cek sebagian
   ...
   */
  it('Harus bisa membuka dan menampilkan detail laporan yang benar dari daftar', () => {
    ...
    //    Tunggu satu input visible untuk memastikan AJAX/load selesai
